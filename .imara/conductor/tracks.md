...
- **001-transparence-des-appels-d-outils-ne-plus-afficher-de-vide-pour-l-utilisateur**: [x] 100% (6 taches realisees : (T1) formatToolAction detecte les arguments vides et affiche "(requete vide)", "(aucun chemin)", etc. (T2) showToolResult affiche un extrait du contenu. (T3) showToolResultWithContent creee dans tool-call.ts avec preview du resultat. (T4) handleToolCall dans agent.ts affiche l'apercu du resultat pour les outils non-read-only. (T5) web-search.tool.ts fait un vrai appel API fallback au lieu d'un message creux. (T6) Build ok, 173/174 tests verts.)
- **021-correction-numerotation-tracks-activation-touche-esc**: [x] 100% (Correction numerotation tracks (max id + 1 au lieu de length + 1) + activation touche Esc (mode raw stdin pour keypress events))
- **022-remplacer-duckduckgo-par-api-imara-pour-web-search-extraction-contenu-sites**: [x] 100% (Remplacer DuckDuckGo par API Imara backend pour web_search + extraction contenu sites. Suppression du package duckduckgo-search et des fichiers de types associes. Ajout du parametre url pour l'extraction de pages web. Correction des tests onboarding pour compatibilite avec le nouveau mode raw stdin.)
- **023-ajout-mode-design-a-web-search-pour-analyse-css-couleurs-polices-frameworks**: [x] 100% (Ajout du mode design a web_search : parametre mode (search|design), detection frameworks (Tailwind, Bootstrap, Bulma...), extraction couleurs, polices, structure DOM, breakpoints, meta, icones. Compilation et tests OK.)
